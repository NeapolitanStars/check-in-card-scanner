    ...
    // Loops through the sessions to find the last session of the card ID and updates the check out time.
        for (let i = sessions.length - 1; i > 0; i--){
            if(sessions[i][0] == cardID && sessions[i][3] == ""){
                ...
                let minutes = (time - checkInTime) / 60000; // Convert milliseconds to minutes
                sessionsSheet.getRange(i + 1, 6).setValue(minutes);
                break;
            }
        }
    }
    // Return the action to the ESP32 so it can be displayed on the LCD screen.
